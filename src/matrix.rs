...
//!
//! The protocol frequently applies Boolean matrices to values known to be in
//! `ker(pi)`.  We store each such value by its 96-coordinate chart, and pack
...
pub const FOUR_RUSSIANS_WIDTH: usize = 4;
...
impl<const OUT: usize> FourRussiansMatrix<OUT> {
    pub fn from_row_masks(input_len: usize, rows: &[u128; OUT]) -> Self {
        assert!(input_len > 0);
        assert_eq!(input_len % FOUR_RUSSIANS_WIDTH, 0);
        assert!(input_len <= u128::BITS as usize);

        let groups = input_len / FOUR_RUSSIANS_WIDTH;
        let mut nibbles = vec![0u8; groups * OUT];
        for group in 0..groups {
            let shift = group * FOUR_RUSSIANS_WIDTH;
            for row_idx in 0..OUT {
                nibbles[group * OUT + row_idx] = ((rows[row_idx] >> shift) & 0x0f) as u8;
            }
        }
        Self { input_len, nibbles }
    }
    ...
    pub fn apply(&self, input: &[Packed4x96], out: &mut [Packed4x96]) {
        ...
        let groups = self.input_len / FOUR_RUSSIANS_WIDTH;
        let mut table = [Packed4x96::default(); 16];
        for group in 0..groups {
            let base = group * FOUR_RUSSIANS_WIDTH;
            fill_table(
                &mut table,
                input[base],
                input[base + 1],
            ...
            let nibbles = &self.nibbles[group * OUT..][..OUT];
            for row_idx in 0..OUT {
                out[row_idx].xor_assign(table[nibbles[row_idx] as usize]);
            }
        }
    }
}
